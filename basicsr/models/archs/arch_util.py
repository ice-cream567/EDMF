...
class EventImage_ChannelAttentionTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor=4, bias=False, LayerNorm_type='WithBias'):
...
#         # high = img + self.ega(img, event) * self.raw_alpha
#
#         return high

class ConvFFN(nn.Module):

    def __init__(self, in_channels, out_channels, expend_ratio=None):
        super().__init__()

        internal_channels = int(in_channels * expend_ratio)
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False):
        ...
        internal_channel = in_channels * 2
        self.relu = nn.GELU()
        self.depthwise = nn.Conv2d(internal_channel, internal_channel, kernel_size=kernel_size, stride=stride, padding=padding,
                                   groups=in_channels, bias=bias)
        self.pointwise = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=bias)
        self.pw1 = nn.Conv2d(in_channels=in_channels, out_channels=internal_channel, kernel_size=1, stride=1,
                             padding=0, groups=1,bias=False)
        self.FFN = ConvFFN(in_channels,in_channels,expend_ratio = in_channels / 32)

        self.conv1 = nn.Conv2d(internal_channel, internal_channel, kernel_size=3, stride=1,padding=1, bias=False)
    def forward(self, x):
        x00 = self.FFN(x)
        x0 = self.pw1(x00)
        ...
        x_1 = self.conv1(x0)
        x_1 = self.relu(x_1)
        x_11 = self.depthwise(x_1)
        x_11 = self.relu(x_11)
        x_111 = self.conv1(x_11)
        x_111 = self.relu(x_111)
        # x_22 = self.pointwise(x_2)
        x_1111, x_2222 = torch.chunk(x_111, 2, dim=1)
        x_0 = x_1111*x_2222
        x_00 = self.pointwise(x_0)
        out = x_00+x
        return out
...
class HF(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=2, bias=False, LayerNorm_type='WithBias'):
        ...
        self.norm1_image = LayerNorm(dim, LayerNorm_type)
        self.norm1_event = LayerNorm(dim, LayerNorm_type)
        self.depthwise_separable_conv = DepthwiseSeparableConv(dim, dim, bias=bias)
    ...
    def forward(self, image, event):
        ...
        assert image.shape == event.shape, 'the shape of image doesnt equal to event'
        b, c, h, w = image.shape
        fused = image + self.depthwise_separable_conv(self.norm1_image(image) * self.norm1_event(event))  # b, c, h, w
